...
A Claude Code skill plugin that lets users search for mentors on MentorCruise.com. It ships one skill (`find-mentor`) backed by an Algolia MCP server for real-time mentor search.
...
To test the skill, run Claude Code from the repo root and try these prompts:
